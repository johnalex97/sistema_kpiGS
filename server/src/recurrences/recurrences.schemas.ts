...
const optionalText = (maximum: number) => z.preprocess(normalizeOptionalString, z.string().trim().max(maximum).optional());
const nullableText = (maximum: number) => z.preprocess(normalizeNullableString, z.string().trim().max(maximum).nullable().optional());
...
const qualityDecisionSchema = z.object({
  ...
  justification: optionalText(1_000),
...
function withPairedCost<T extends z.ZodType>(schema: T): T {
  return schema.superRefine((value, context) => {
    const record = value as { estimatedCost?: string; costReason?: string };
    const hasCost = record.estimatedCost !== undefined;
    const hasReason = record.costReason !== undefined;
...
export const addRecurrenceVisitSchema = z.object({ version: positiveVersion, orderId: z.uuid(), observation: optionalText(10_000) }).strict();
...
export const adjustRecurrenceSchema = withPairedCost(z.object({
  ...
  estimatedCost: costSchema.optional(),
  costReason: optionalText(500),
}).strict().refine((value) => Object.entries(value).some(([key, fieldValue]) => key !== "version" && key !== "reason" && fieldValue !== undefined), {
