...
export function TechnicianDetail({ technician, kpi, showKpi, actions = [], onClose, onAction }: TechnicianDetailProps) {
  ...
  return <aside className="technician-detail" role="dialog" aria-labelledby="technician-detail-title" aria-modal="false" tabIndex={-1} ref={panelRef}>
    <header className="technician-detail__head"><div><p className="eyebrow">Ficha laboral · v{technician.version}</p><h2 id="technician-detail-title">Detalle del técnico</h2></div><button className="icon-button" type="button" aria-label="Cerrar detalle" onClick={onClose}><X size={18} /></button></header>
