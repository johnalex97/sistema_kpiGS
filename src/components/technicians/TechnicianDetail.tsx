...
export function TechnicianDetail({ technician, kpi, showKpi, actions = [], onClose, onAction }: TechnicianDetailProps) {
  ...
  return <aside className="technician-detail" role="dialog" aria-labelledby="technician-detail-title" aria-modal="false" tabIndex={-1} ref={panelRef}>
    ...
    {actions.length > 0 && <footer className="technician-detail__actions">{actions.map((action) => <button key={action} data-technician-action={action} className={`button ${action === "edit" || action === "reactivate" ? "button--primary" : "button--ghost"}`} type="button" onClick={() => onAction(action)}>{actionLabel[action]}</button>)}</footer>}
  </aside>;
}
