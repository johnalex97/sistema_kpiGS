...
export function OrderMaterials({ order, catalog, canManage, pending, error, onAdd, onUpdate, onRemove }: OrderMaterialsProps) {
  ...
  useEffect(() => {
    if (!removing) return;
    removalDialogRef.current?.querySelector<HTMLElement>("button:not([disabled])")?.focus();
  }, [removing]);
  ...
  const confirmRemoval = async () => {
    if (removing && await onRemove(removing.id)) closeRemoval();
  };
